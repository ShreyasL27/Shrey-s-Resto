...
const ItemList = ({ items }) => {
    ...
    return (
        <div className="border-[rgb(239,239,245)] border-b-[15px]">
            {items.map((item) => (
                <div data-testid = "foodItems" key={item?.card?.info?.id} className="flex justify-between list-none px-6 border-b-2">
                    <div className="w-9/12 my-6">
                        <li className="font-medium">{item?.card?.info?.name}</li>
                        <li className="text-sm">₹{item?.card?.info?.price ? item?.card?.info?.price / 100 : item?.card?.info?.defaultPrice / 100}</li>
                        <p className="my-3 text-sm text-gray-400">{item?.card?.info?.description}</p>
                    </div>
                    <div className="p-5 w-3/12">
                        ...
                        <img src={CDN_URL + item?.card?.info?.imageId} alt="NA" className="h-24 w-full rounded-lg" />
                    </div>
                </div>
            ))}
        </div>
